...
def format_link_status_bar(snapshot: LinkSnapshot, *, frame: int = 0) -> str:
    ...
    if state == "idle":
        return (
            f"[green]●[/] [green]連線正常[/]"
            f"  [dim]│[/]  [dim]待命[/]"
            f"  [dim]│[/]  [dim]上次 {_format_age(recv_age)}[/]"
        )
    rtt_part = ""
    if snapshot.last_rtt_ms is not None:
        rtt_part = f"  [dim]│[/]  [dim]延遲 {int(snapshot.last_rtt_ms)}ms[/]"
    return (
        f"[green]●[/] [green]連線順暢[/]"
        f"{rtt_part}"
        f"  [dim]│[/]  [dim]待命[/]"
        f"  [dim]│[/]  [dim]更新 {_format_age(recv_age)}[/]"
    )
