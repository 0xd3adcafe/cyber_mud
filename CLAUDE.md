...
| **Traditional Chinese locale (ZH)** | ZH.1–ZH.6 shipped — `shared/zh_traditional_audit.py`, `shared/locale_validate.py`, `admin.sh validate` hard-fail; see PHASES **Traditional Chinese locale quality** |
...
| Mature / NSFW content M.0–M.17 | `mature_social.py`, mature say/presence/gifts, combat broadcasts, `taunt`/`finish` |
...
| PDA integrated growth panel | Single `pda` with proficiencies + talent catalog; full `build_pda_ui` sidebar; `tests/test_pda.py` |
