...
| ~~M.19~~ | ~~內容包文風指南~~ | ✅ `data/mature/docs/STYLE.md`＋`STYLE.zh.md`；noir／lewd 規則；禁用陳腔；`{persona}`／`{player}` 模板 |
| ~~M.20~~ | ~~雙聲線解析~~ | ✅ `world/mature_voice.py`；`resolve_mature_voice()` → `noir`｜`lewd`；`data/mature/voice.yaml`；`mature.noir.*`／`mature.lewd.*` |
| ~~M.21~~ | ~~Persona（SFW，全玩家）~~ | ✅ `persona`／`persona set`／`persona clear`；`Player.persona` 存檔；`look <玩家>` 公開；`tests/test_persona.py` |
| ~~M.22~~ | ~~Scene＋whisper~~ | ✅ `scene`／`scene status`；`whisper <目標> <訊息>`；擴充 `romance.yaml`；`tests/test_mature_social.py` |
| ~~M.23~~ | ~~Client 成熟 Rich~~ | ✅ `client/mature_format.py`；`*動作*`／對白／`>環境`／SFX 色盤；`mature` log channel；`@meta mature_voice` 狀態晶片；`tests/test_mature_format.py` |
...
**原則：** 原創 Night City 致敬——僅借 [Bleachbunny / Bunnyverse](https://bleachbunny.net/) 與 [chub.ai/users/bleachbunny](https://chub.ai/users/bleachbunny) 的**原型與互動節奏**（Arcana、Families、CYOA、Wintr、Slutbunny **文風**），**不**複製卡內人名、劇情或 Arcade／遊戲 IP。成熟文案在 **`cyber_mud_mature`**。浪漫**階段**僅靠 `flirt`／`spend_time` 推進（`whisper`、`say`、`give`、委託結算不推階段）。

**設計決策（2026-06，已確認）：**

| 決策 | 選擇 |
|------|------|
| 區域覆蓋 | **四線全做：** Kabuki 娛樂、Watson Trauma、Tyrell 企業、Watson 合租＋Little China 寄宿 |
| Arcana | **`interact` 塔羅抽牌**（22 大阿卡納語系鍵，非 66 個實體 NPC） |
| 偶像線 | **Haejin ↔ Airi** 互連成熟委託＋雙結局（`quest_flags`） |
| Trauma Team | **BB.2 實作優先**（CYOA 致敬→ Watson 診所循環） |
| Lewd 密度 | 新成熟浪漫 NPC 約 **50%** 設 `voice_default: lewd` 或 `voice_override: lewd` |
