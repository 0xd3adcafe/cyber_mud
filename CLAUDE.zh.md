...
本 repo 交付可遊玩的**夜城**堆疊：Python 伺服器、Textual client、YAML 世界資料、測試與完整實作文件（見 `docs/`）。
...
（`saves`／`delete-save` 管理指令等待 persistence 實作後補上。）
...
| 目標消歧 TD.1–TD.5 | `shared/target_resolve.py`；物品／NPC／環境／商店指令統一序號與範圍 |
...
| **Client log 可讀性** | CL.1–CL.11 已交付——log 頻道、`LogPalette`、`/log` 精簡與隱藏、匯出；見 PHASES **Client 主 log 可讀性** |
...
| 成人／NSFW 內容 M.0–M.17 | `mature_social.py`、mature say/presence/送禮、戰鬥廣播、`taunt`/`finish` |
...
| PDA 整合成長面板 | 單一 `pda` 含熟練度與天賦目錄；完整 `build_pda_ui` 側欄；`tests/test_pda.py` |

**架構增量（已交付）：** ARCH.1 Lock YAML 條件、ARCH.2 Scheduler 薄層 — 見 PHASES **架構（增量）**。

**監控駭客（Watch Dogs 致敬）：** WD.1–WD.10 已交付 — Profiler、ctOS 基礎設施駭入、數位足跡、DedSec — 見 PHASES **監控駭客**。

**Hacknet 風格 NETRUN（HN）：** HN.1–6 已交付 — trace 追蹤條、connect/breach/exploit、環境斷線、節點檔案、mesh 路由 — 見 PHASES **Hacknet 風格 NETRUN**。

**M.18（已交付）：** private `cyber_mud_mature` submodule（`data/mature/`）；內容包雙語文件（`README`、`CLAUDE`、`CONTRIBUTING`、`LOCALIZATION` + `*.zh.md`）；`scripts/mature-history-purge.sh` 清 history — 見 PHASES **M.18**、[docs/MATURE_CONTENT.zh.md](docs/MATURE_CONTENT.zh.md)。

限制級文案位於 private **`cyber_mud_mature`**（`data/mature/locale/mature_en.yaml`、`mature_zh.yaml`）；預設 `teen`；M.0–M.18 已交付。
